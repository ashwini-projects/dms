...
routes.post('/list', function (req, res, next) {
  ...
  var promise;
  var self = this;
  console.log("basedir is: ", pathResolver.baseDir(req))
...
routes.post('/createFolder', upload.any(), function (req, res, next) {
  ...
  var folderPath = path.join(pathResolver.baseDir(req), pathResolver.pathGuard(req.body.path + req.body.newPath));

  var promise = mkdirp(folderPath, (err) =>{
